...
class _AdminFilterModalState extends State<AdminFilterModal> {
  ...
  void initState() {
    ...
    _minAreaController = TextEditingController(
      text: widget.currentFilters['minArea'],
    );
    _maxAreaController = TextEditingController(
      text: widget.currentFilters['maxArea'],
    );
  }
  ...
  Widget _buildModalDropdown(
    ...
    String safeValue = values.contains(currentValue)
        ? currentValue
  ...
  Widget build(BuildContext context) {
    List<String> pTypeValues = [
      'all',
      ...widget.projectTypes.map((e) => e['id'].toString()),
    ];
    List<String> pTypeDisplays = [
      'ทุกประเภท',
      ...widget.projectTypes.map((e) => e['name'].toString()),
    ];
    List<String> pCatValues = [
      'all',
      ...widget.productCategories.map((e) => e['id'].toString()),
    ];
    List<String> pCatDisplays = [
      'ทุกสินค้า',
      ...widget.productCategories.map((e) => e['name'].toString()),
    ];
        ...
        children: [
          ...
          Row(
            ...
            children: [
              const Row(
                children: [
                  Icon(Icons.tune_rounded, color: kPremiumGold, size: 24),
                  SizedBox(width: 10),
                  Text(
                    "ตัวกรองข้อมูลเชิงลึก",
                    style: TextStyle(
                      color: Colors.white,
                      fontSize: 20,
                      fontWeight: FontWeight.bold,
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  _buildFilterSection(
                    title: "ทีมงาน & ผู้รับผิดชอบ",
                    icon: Icons.groups_rounded,
                    child: Row(
                      children: [
                        Expanded(
                          child: _buildModalDropdown(
                            ['all', ...widget.availableTeams],
                            ['ทุกทีม', ...widget.availableTeams],
                            _selectedTeam,
                            (val) => setState(() {
                              _selectedTeam = val!;
                              _selectedPerson = 'all';
                        ...
                        Expanded(
                          child: _buildModalDropdown(
                            ['all', ...widget.availablePersons],
                            ['ทุกคน', ...widget.availablePersons],
                            _selectedPerson,
                            (val) => setState(() {
                              _selectedPerson = val!;
                              _selectedTeam = 'all';
