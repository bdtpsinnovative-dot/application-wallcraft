...
class AddVisitModal extends StatefulWidget {
  ...
  final Future<void> Function(Map<String, dynamic> visitData) onSave;
...
class _AddVisitModalState extends State<AddVisitModal> {
  ...
  String? _projectHistoryKey;
  List<dynamic> _companyProjectHistory = [];
  ...
  void initState() {
    ...
    if (widget.initialData != null) {
      _clientRequestId = widget.initialData!['client_request_id']?.toString();
      ...
      final savedStartTime = _parseTime(widget.initialData!['start_time']);
      final savedEndTime = _parseTime(widget.initialData!['end_time']);
      if (savedStartTime != null) _startTime = savedStartTime;
      if (savedEndTime != null) _endTime = savedEndTime;
      ...
      if (widget.initialData!['project_type_id'] != null) {
        _selectedProjectType = widget.initialData!['project_type_id']
            .toString();
      }
      if (widget.initialData!['product_category_id'] != null) {
        _selectedCategory = widget.initialData!['product_category_id']
            .toString();
      }
      ...
      if (_selectedDate.isBefore(DateTime.now()) &&
        ...
        _selectedDate =
            DateTime.now(); // If it's current week, default to today
      }
    }
    _fetchData();
  }
  ...
  Future<void> _fetchData() async {
    try {
      ...
      final responses = await Future.wait([
        ...
        ), // 🟢 ดึง project_types, product_categories และ initial projects รวดเดียว
      ]);
      ...
      if (responses[1].statusCode == 200) {
        ...
        _customerTypes = decoded['customer_types'] ?? [];
    ...
    final picked = await showDialog<TimeOfDay>(
      ...
      builder: (dialogContext) => StatefulBuilder(
        builder: (context, dialogSetState) => AlertDialog(
          ...
          title: Text(
            isStart ? 'เลือกเวลาเริ่มเข้าพบ' : 'เลือกเวลาสิ้นสุดการเข้าพบ',
          ...
          content: Row(
            ...
            children: [
              _timeDropdown(
                ...
                onChanged: (value) =>
                    dialogSetState(() => selectedHour = value),
              ),
              const Padding(
              ...
              _timeDropdown(
                ...
                onChanged: (value) =>
                    dialogSetState(() => selectedMinute = value),
              ),
        ...
        child: Container(
          ...
          child: Row(
            ...
            children: [
              ...
              const Icon(Icons.access_time, color: Colors.white54, size: 19),
            ],
          ),
        ),
      ),
  ...
  Future<void> _submitPlan() async {
    ...
    final data = {
      ...
      'start_time': _timeForApi(_startTime),
      'end_time': _timeForApi(_endTime),
      ...
      if (widget.isAdmin && _selectedAssignToUserId != null)
        'user_id': _selectedAssignToUserId,
    };
    if (widget.initialData != null) {
  ...
  Future<List<dynamic>> _getCompanyOptions(String filter) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        var apiCompanies = decoded is List
            ? decoded
            : (decoded['data'] ?? decoded['companies'] ?? []);
  ...
  Future<List<dynamic>> _getCompanyProjectHistory() async {
    if (_selectedCompany == null) return [];

    final companyId = _selectedCompany!['id']?.toString();
    if (companyId == null || companyId.isEmpty) return [];
    final targetUserId = widget.isAdmin ? _selectedAssignToUserId : null;
    final cacheKey = '${targetUserId ?? 'self'}|$companyId';
    if (_projectHistoryKey == cacheKey) return _companyProjectHistory;

    try {
      final response = await ApiService.getVisitPlanProjectHistory(
        companyId: companyId,
        userId: targetUserId,
      );
      if (response.statusCode == 200) {
        final decoded = jsonDecode(response.body);
        _companyProjectHistory = decoded is Map && decoded['projects'] is List
            ? List<dynamic>.from(decoded['projects'])
            : [];
        _projectHistoryKey = cacheKey;
      }
    } catch (e) {
      debugPrint("Error fetching company project history: $e");
    }
    return _companyProjectHistory;
  }

  // Helper to merge company history and API search projects
  Future<List<dynamic>> _getProjectOptions(String filter) async {
    ...
    // This endpoint is used only by the new visit-plan form. It checks the
    // selected salesperson's history for the selected company.
    var companyProjects = await _getCompanyProjectHistory();
    if (companyProjects.isEmpty && _selectedCompany?['isPipeline'] == true) {
      companyProjects = List<dynamic>.from(_selectedCompany!['projects'] ?? []);
    }

    // Put previously used projects into the starred group first.
    for (var p in companyProjects) {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        var decoded = jsonDecode(response.body);
        var apiProjects = decoded is List
            ? decoded
            : (decoded['data'] ?? decoded['projects'] ?? []);
    ...
    // Starred projects first, then other company projects, then general search.
    final orderedOptions = <dynamic>[
      ...mineOptions,
      ...othersOptions,
      ...options,
      ...unusedOptions,
    ...
    if (_selectedProject != null) {
      ...
      if (!orderedOptions.any((o) => o['id'].toString() == spId)) {
        orderedOptions.insert(0, _selectedProject!);
      }
    }
  ...
  void _onProjectSelected(dynamic val) {
    setState(() {
      ...
      if (val != null) {
        if (val['project_type_id'] != null)
          _selectedProjectType = val['project_type_id'].toString();
        if (val['product_category_id'] != null)
          _selectedCategory = val['product_category_id'].toString();
      }
    });
  }

  Future<void> _addNewCompany(String name, String? customerTypeId) async {
    try {
      final response = await ApiService.post(
        Uri.parse('${AppConfig.baseUrl}/companies'),
        body: jsonEncode({'name': name, 'customer_type_id': customerTypeId}),
      );
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        setState(() {
          ...
          _projectHistoryKey = null;
          _companyProjectHistory = [];
        });
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
        ...
        throw Exception('เพิ่มบริษัทไม่สำเร็จ (${response.statusCode})');
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<bool> _addNewProject(String name) async {
    try {
      final response = await ApiService.post(
      ...
      if (response.statusCode != 200 && response.statusCode != 201)
        return false;
      final project = Map<String, dynamic>.from(jsonDecode(response.body));
          ...
          actions: [
            ...
            TextButton(
              onPressed: () {
                ...
                _addNewCompany(name, selectedTypeId);
              },
              child: const Text(
                'บันทึก',
                style: TextStyle(
      ...
      builder: (dialogContext) => StatefulBuilder(
        builder: (context, setDialogState) => AlertDialog(
          ...
          content: TextField(
            ...
            autofocus: true,
            style: const TextStyle(color: Colors.white),
            decoration: _dialogInputDecoration('ชื่อโครงการ *'),
          ),
          actions: [
            TextButton(
            ...
            TextButton(
              onPressed: isSaving
                  ? null
                  : () async {
                      final name = projectNameCtrl.text.trim();
                      if (name.isEmpty) return;
                      setDialogState(() => isSaving = true);
                      final success = await _addNewProject(name);
  ...
  Widget _addInlineButton(VoidCallback onTap) {
    return SizedBox(
      width: 44,
      height: 44,
              ...
              Container(
                ...
                child: const Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(
                        "แผนนี้เสร็จสิ้นแล้ว ไม่สามารถแก้ไขได้",
                        style: TextStyle(
                          color: Colors.greenAccent,
                          fontWeight: FontWeight.bold,
                          fontSize: 13,
                    ...
                    child: SingleChildScrollView(
                      child: Column(
                        ...
                        children: [
                          ...
                          if (widget.isAdmin &&
                              widget.adminUsersList.isNotEmpty) ...[
                            DropdownButtonFormField<String>(
                              isExpanded: true,
                          ...
                          Row(
                            ...
                            children: [
                              ...
                              const SizedBox(width: 8),
                              _addInlineButton(_showAddCompanyDialog),
                            ],
                          ),
                          const SizedBox(height: 16),
                          ...
                          Row(
                            ...
                            children: [
                              ...
                              const SizedBox(width: 8),
                              _addInlineButton(_showAddProjectDialog),
                            ],
                          ),
                          const SizedBox(height: 16),
                          ...
                          TextField(
                            ...
                            decoration: InputDecoration(
                              ...
                              fillColor: kCardDark,
                              hintText:
                                  "รายละเอียดเพิ่มเติม เช่น โปรเจ็กต์โรงแรม 5 ดาว...",
                ...
                child: Container(
                  ...
                  child: const Row(
                    children: [
                      ...
                      SizedBox(width: 8),
                      Text(
                        "รายการนี้เสร็จสิ้นแล้ว ไม่สามารถแก้ไขได้",
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                onPressed: _isReadOnly
                    ...
                    : (_selectedCompany == null || _isSaving
                          ? null
                          : _submitPlan),
                style: ElevatedButton.styleFrom(
